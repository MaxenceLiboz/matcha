...
export class VerificationRepository
  extends AbstractRepositoryImpl<Verification, "Verification">
  implements IVerificationRepository
{
  ...
  async save(verification: Verification): Promise<Verification> {
    if (verification.id) {
      // Update if verification already exist
      const updateData: Updateable<VerificationTable> =
        VerificationMapper.toPersistenceUpdate(verification);
      await this.db
      ...
      if (new_verification == null) {
        throw new CustomError(
          "Error occured while getting the updated verification.",
      ...
      // Insert otherwise
      const insertData: Insertable<VerificationTable> =
        VerificationMapper.toPersistenceInsert(verification);
      ...
      const new_verification = await this.getById(
        Number(insertedRecord.insertId),
      );

      if (new_verification == null) {
        throw new CustomError(
          "Error occured while getting the created verification.",
          HTTP_STATUS.UNPROCESSABLE_ENTITY,
        );
      }
      return new_verification;
