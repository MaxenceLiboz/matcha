...
> implements IAbstractRepository<TDomain>
{
  ...
  async getById(id: number): Promise<TDomain | null> {
    if (id === null || id === undefined) {
      throw new CustomError(
        "ID must not be null or undefined.",
        HTTP_STATUS.BAD_REQUEST,
      );
    }

    const { rows: results } = await sql<
      Selectable<DB[TTableName]>
    >`SELECT * FROM ${sql.table(this.tableName)} WHERE id = ${id}`.execute(
      this.db,
    );
  ...
  async getByFields(fields: { [filedName: string]: any }): Promise<TDomain[]> {
    ...
    if (Object.keys(fields).length > 0) {
      ...
      finalQuery = sql<
        Selectable<DB[TTableName]>
      >`SELECT * FROM ${sql.table(this.tableName)} WHERE ${sql.join(conditions, sql<Selectable<DB[TTableName]>>` AND `)}`;
    } else {
      finalQuery = sql<
        Selectable<DB[TTableName]>
