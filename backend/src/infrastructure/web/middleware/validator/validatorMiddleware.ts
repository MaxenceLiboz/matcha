...
export default function validationMiddleware(
  ...
  dataLocation: "body" | "query" | "params" = "body",
) {
    ...
    switch (dataLocation) {
      ...
      default:
        throw new CustomError(
          `Invalid data location specified for validation`,
...
function validateData(
  ...
  if (!dataToValidate) {
    ...
    for (const fieldName in schema) {
      ...
      const isExplicitlyRequired = rules.some(
        (rule) => rule.type === VALIDATOR.REQUIRED,
      );
  ...
  for (const fieldName in schema) {
    ...
    const isFieldRequired = rules.some(
      (rule) => rule.type === VALIDATOR.REQUIRED,
    );
    const isValueEmpty =
      value === undefined ||
    ...
    for (const rule of rules) {
      ...
      if (!validatorFn) {
        console.warn(
          `Unknown validator type: ${rule.type} for field ${fieldName}`,
        );
        continue;
      }
