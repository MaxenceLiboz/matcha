  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!filesByField || Object.keys(filesByField).length === 0) {
      ...
      if (options.minTotalFileCount && options.minTotalFileCount > 0) {
        return next(
          new CustomError(
            `At least ${options.minTotalFileCount} picture(s) required.`,
      ...
      if (options.requiredFields && options.requiredFields.length > 0) {
        return next(
          new CustomError(
            `Required picture field '${options.requiredFields[0]}' is missing.`,
            HTTP_STATUS.BAD_REQUEST,
          ),
        );
      }
    ...
    if (options.requiredFields) {
      for (const fieldName of options.requiredFields) {
        if (!filesByField[fieldName] || filesByField[fieldName].length === 0) {
          return next(
            new CustomError(
              `Picture field '${fieldName}' is required.`,
              HTTP_STATUS.BAD_REQUEST,
            ),
          );
        }
    ...
    for (const fieldName in filesByField) {
      ...
      for (const file of files) {
        ...
        if (file.size > options.maxSize) {
          return next(
            new CustomError(
              `File ${file.originalname} is too large. Max size is ${
        ...
        if (!options.allowedMimeTypes.includes(file.mimetype)) {
          return next(
            new CustomError(
              `File type for ${
                file.originalname
              } is not allowed. Allowed types: ${options.allowedMimeTypes.join(
                ", ",
              )}.`,
              HTTP_STATUS.BAD_REQUEST,
            ),
    ...
    if (options.minTotalFileCount && totalFiles < options.minTotalFileCount) {
      return next(
        new CustomError(
          `At least ${options.minTotalFileCount} total picture(s) required.`,
          HTTP_STATUS.BAD_REQUEST,
        ),
      );
    }
