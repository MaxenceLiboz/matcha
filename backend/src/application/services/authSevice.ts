...
export class AuthService {
  ...
  async sendUserVerification(
    data: SendUserVerificationOrForgotPasswordDTO,
  ): Promise<void> {
  ...
  async verifyUser(data: UserVerificationOrForgotPasswordDTO): Promise<void> {
    const verification =
      await this.verificationUseCases.getValidVerificationByToken(data.token);

    // If token found is not good type throw
    if (verification.type != "email_verification") {
      throw new CustomError(
        "The token given is not an email verification token",
        HTTP_STATUS.BAD_REQUEST,
      );
    }
