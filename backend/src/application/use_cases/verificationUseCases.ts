...
export class VerificationUseCases {
  ...
  async createVerification(verification: Verification): Promise<Verification> {
    if (
      ...
      !verification.unique_token ||
      !verification.type ||
      !verification.expiration_date
    ) {
      throw new CustomError(
        "Verification entity can´t be create without all informations",
        HTTP_STATUS.UNPROCESSABLE_ENTITY,
      );
    }

    const old_verifs = await this.verificationRepository.getByUserIdAndType(
      verification.user_id,
      verification.type,
    );
  ...
  async getValidVerificationByToken(token: string): Promise<Verification> {
    ...
    const verification =
      await this.verificationRepository.getValidByToken(token);
    if (verification == null) {
      throw new CustomError(
        "No valid verification found with given token",
  ...
  async expireVerification(verification: Verification): Promise<void> {
    if (!verification.id) {
      throw new CustomError(
        "Can´t exprired verification without its id",
        HTTP_STATUS.UNPROCESSABLE_ENTITY,
      );
    }
